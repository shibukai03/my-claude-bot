...
def main():
    logger.info("=" * 60)
    logger.info("映像案件スクレイピング v1.27 [全角・和暦完全対応・最終版]")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("【ステップ2】案件選別（不純物を排除し、全角/半角問わず本物を救済）...")
        for i, task in enumerate(all_tasks, 1):
            ...
            # --- 🛡️ 門番3：年度検閲（令和7, 8, 9年度を幅広く救済） ---
            # 1月は次年度(R8)の公募が始まる時期なので、R7予算のR8案件も含めて許可
            ...
            # ② 令和8年度(2026)の案件であることをAIの言葉から最終確認
            # AIが「令和7年度です」と言い切っているものは、不純物として排除
            if re.search(r"令和7年度?の案件|令和7年度のみ", memo) and "令和8" not in full_ans:
                continue
